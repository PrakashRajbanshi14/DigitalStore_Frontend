...
import { handleCategoryItemDelete, updateCategory } from "../../../../store/adminCategorySlice"
...
function CategoryTable({categories}:{categories : ICategory[]}){
  ...
  const handleUpdateCategory = async (e: FormEvent<HTMLFormElement>) => {
    ...
    setSavingEdit(true)
    const success = await dispatch(updateCategory(editingCategory.id, editCategoryName))
    if (success) {
      closeEditModal()
    } else {
      setSavingEdit(false)
  ...
  const closeModal = useCallback( ()=>setIsModalOpen(false),[])
    ...
    return ( 
        ...
        <div className=" overflow-x-auto">
          ...
          {isEditModalOpen && (
            <div id="edit-modal" className="fixed inset-0 z-50 flex items-center justify-center">
              <div className="fixed inset-0 bg-black/50" />
              <div className="relative w-full max-w-md p-6 bg-white dark:bg-gray-800 rounded-lg shadow-xl">
                <div className="flex items-center justify-between mb-4">
                  <h3 className="text-lg font-semibold text-gray-900 dark:text-white">Edit Category</h3>
                  <button onClick={closeEditModal} className="text-gray-500 hover:text-gray-700 dark:text-gray-300">
                    ×
                  </button>
                </div>
                <form onSubmit={handleUpdateCategory} className="space-y-4">
                  <div>
                    <label htmlFor="edit-category-name" className="block text-sm font-medium text-gray-700 dark:text-gray-300">Category Name</label>
                    <input
                  ...
                  <div className="flex justify-end gap-3">
                    <button type="button" onClick={closeEditModal} className="px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-md hover:bg-gray-50 dark:bg-gray-700 dark:text-gray-300 dark:border-gray-600 dark:hover:bg-gray-600">
                      Cancel
                    </button>
                    <button type="submit" className="px-4 py-2 text-sm font-medium text-white rounded-md bg-indigo-600 hover:bg-indigo-700" disabled={savingEdit || !editCategoryName.trim()}>
          ...
          <div className="min-w-full inline-block align-middle">
            <div className="relative  text-gray-500 focus-within:text-gray-900 mb-4">
             ...
             <input onChange={(e)=>setSearchTerm(e.target.value)} type="text" id="default-search" className="block w-80 h-11 pr-5 pl-12 py-2.5 text-base font-normal shadow-xs text-gray-900 bg-transparent border border-gray-300 rounded-full placeholder-gray-400 focus:outline-none" placeholder="Search" />
              <button className="bg-blue-500 rounded text-white p-2" onClick={openModal}>+ Category</button>
             </div>
            </div>
                ...
                <tbody className="divide-y divide-gray-300 ">
                  {
                    ...
                    <td className=" p-5 ">
                      <div className="flex items-center gap-1">
                        <button type="button" onClick={()=>openEditModal(category)} className="p-2 rounded-full bg-indigo-100 text-indigo-600 hover:bg-indigo-200 transition-all duration-300 flex items-center">
                        ...
                        <button onClick={()=>deleteCategory(category?.id)} className="p-2 rounded-full  group transition-all duration-500  flex item-center">
